"""
NWOA-Nelder-Mead Hybrid Algorithm

Combines Narwhal Optimization with Nelder-Mead Simplex local search.
Nelder-Mead is applied to the best solutions periodically for local refinement.

Advantages:
- Excellent for smooth, continuous functions
- Low computational overhead
- Robust derivative-free method
- Good convergence on unimodal landscapes
...
class NWOA_NelderMead:
    """
    Hybrid NWOA with Nelder-Mead local search.
    ...
    def nelder_mead_search(self, start_point: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        Robust derivative-free optimization for smooth functions.
        """
    ...
    def optimize(self) -> Tuple[np.ndarray, float, List[float]]:
        """Run NWOA-Nelder-Mead optimization."""
        ...
        for t in range(self.max_iter):
            ...
            # Apply Nelder-Mead local search periodically
            if (t + 1) % self.ls_frequency == 0:
                sorted_indices = np.argsort(self.fitness)
                best_indices = sorted_indices[:self.ls_candidates]
                
                for idx in best_indices:
                    refined_solution, refined_fitness = self.nelder_mead_search(self.agents[idx])
