"""
NWOA-Pattern Search Hybrid Algorithm

Combines Narwhal Optimization with Pattern Search (Hooke-Jeeves) local refinement.
Pattern Search uses coordinate descent for systematic local exploration.

Advantages:
- Very effective for separable problems
- Systematic exploration along coordinate axes
...
class NWOA_PatternSearch:
    """
    Hybrid NWOA with Pattern Search local refinement.
    ...
    def pattern_search(self, start_point: np.ndarray) -> Tuple[np.ndarray, float]:
        """
        Pattern Search (Hooke-Jeeves) local refinement.
        Systematic coordinate descent along each dimension.
        """
        ...
        while evals_used < self.ls_max_evals:
            ...
            if improved:
                step_size *= 1.2  # Increase step on success
            else:
                step_size *= 0.5  # Decrease step on failure
    ...
    def optimize(self) -> Tuple[np.ndarray, float, List[float]]:
        """Run NWOA-Pattern Search optimization."""
        ...
        for t in range(self.max_iter):
            ...
            # Apply Pattern Search periodically
            if (t + 1) % self.ls_frequency == 0:
                sorted_indices = np.argsort(self.fitness)
                best_indices = sorted_indices[:self.ls_candidates]
                
                for idx in best_indices:
                    refined_solution, refined_fitness = self.pattern_search(self.agents[idx])
