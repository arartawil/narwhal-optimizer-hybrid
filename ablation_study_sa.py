"""
Ablation Study for NWOA-SA (Simulated Annealing Hybrid)
========================================================
  1. NWOA (base, no LS)
  2. NWOA-SA Full (freq=10, cand=3, budget=50)
  3. NWOA-SA (freq=50 — less frequent LS)
  4. NWOA-SA (cand=1 — refine only the best)
...
class NWOA_SA_Full:
    """NWOA-SA Full: freq=10, cand=3, budget=50."""
    def __init__(self, pop_size, dim, lb, ub, max_iter, obj_func):
...
class NWOA_SA_FreqLow:
    """NWOA-SA with less frequent LS: freq=50."""
    def __init__(self, pop_size, dim, lb, ub, max_iter, obj_func):
        self.algo = NWOA_SimulatedAnnealing(
            objective_function=obj_func, dim=dim, lb=lb, ub=ub,
            n_agents=pop_size, max_iter=max_iter,
...
class NWOA_SA_Cand1:
    """NWOA-SA refining only the best solution: cand=1."""
    def __init__(self, pop_size, dim, lb, ub, max_iter, obj_func):
        self.algo = NWOA_SimulatedAnnealing(
            objective_function=obj_func, dim=dim, lb=lb, ub=ub,
            n_agents=pop_size, max_iter=max_iter,
...
def build_ablation_suite(dim):
    ...
    return BenchmarkSuite(category="CEC2022_Ablation_SA", benchmarks=benchmarks)


if __name__ == "__main__":
    algorithms = [
        ("NWOA",            NWOA_Base),
        ("NWOA-SA_Full",    NWOA_SA_Full),
        ("NWOA-SA_Freq50",  NWOA_SA_FreqLow),
        ("NWOA-SA_Cand1",   NWOA_SA_Cand1),
    ]
    suites = [build_ablation_suite(DIM)]
    run_experiment(
        algorithms=algorithms, benchmark_suites=suites,
        output_dir="Hybrid/Ablation_Study_SA",
